...
const { nom_RoleSondages, id_NotifSondages, id_DiscordGarden } = require('../config.json');
...
exports.run = async (client, message, args, tools) => {
    ...
    if (argus) {
      if (!canSendPoll(message.author.id)) {
        ...
        return message
          .channel.send(yesno_embed).then(message.channel.send(`<@&${id_NotifSondages}>`)).then(async (pollMessage) => {
        ...
        return message.channel.send(multiple_embed)
          .then(message.channel.send(`<@&${id_NotifSondages}>`))
        ...
        message.delete({timeout: 2000}); // This waits 3000 milliseconds before deleting (2 seconds)
            });
        }
    }
}
    else {
      return message.channel.send(`**Erreur:** ${message.author}, la question et les options doivent être entre guillemets (**"**). Pour plus d'infos, faites ` + "`!npoll`.");
    }
  } 
}